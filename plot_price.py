...
def execute_plot(subsetBeforeAlgoVar, subsetAfterAlgoVar):
    ...
    fig, ax = plt.subplots()
    plt.plot(subsetBeforeAlgoVar['Time'], subsetBeforeAlgoVar['Price'], label='Market Price (BA)', alpha=0.6, linewidth = 1.3)
    plt.plot(subsetAfterAlgoVar['Time'], subsetAfterAlgoVar['Price'], label='Market Price (AA)', alpha=0.6, linewidth = 1.3)
